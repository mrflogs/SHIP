...
def run_vae_generator(cfg, dataset, cache_keys, cache_values, clip_weights, clip_model): 
    # need to evaluate new dataset.
    # CLIP
    ...
    prompt_learner = PromptLearner(dataset.classnames, clip_model).float().cuda()
    
    # test on new dataset.
    # global base&new val/test data.
    ...
    val_features_new, val_labels_new = pre_load_features(cfg, "val", clip_model, val_loader_new)
    test_features_new, test_labels_new = pre_load_features(cfg, "test", clip_model, test_loader_new)
    
    train_loader_F_new = build_data_loader(data_source=dataset_new.train_x, batch_size=256, tfm=train_tranform, is_train=True, shuffle=True)
    print("\nGetting textual features as CLIP's classifier.")
    clip_weights_new = clip_classifier(dataset_new.classnames, dataset_new.template, clip_model.float())
    
    global base_classnames
    global new_classnames
    global all_classnames
...
def main():
    ...
    run_vae_generator(cfg, dataset, cache_keys, cache_values, clip_weights, clip_model)
